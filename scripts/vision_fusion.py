...
def cross_validate(pid, vision_result):
    ...
    if vision_spec == geo_spec:
        ...
        verdict.update({'状态': '冲突', '待核': True,
                        '裁决': f'视觉({vision_spec})与几何识图({geo_spec})不一致, 需人工复核'})
    return verdict
